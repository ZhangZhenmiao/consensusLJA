...
def run_minimap2_to_bam(flank_fasta, hifi_reads, output_bam):
    cmd = f"/Poppy/zmzhang/Consensus_Assembly/cLJA/lib/LJA/bin/compress --dimer-compress 32,32,1 --reads {hifi_reads} | minimap2 -ax map-hifi -Y --eqx --sam-hit-only --secondary=no -t 80 {flank_fasta} - | samtools sort -@ 80 -o {output_bam}"
        ...
        for alignment in bamfile:
            ...
            if identity >= threshold:
                ...
                if query_alignment_end - query_alignment_start >= 3000:
                    ...
                    node1, node2, _ = query_name.split("_")
                    if node1 == node2:
                        continue
...
def summarize_full_connected_sequences(high_identity_alignments, ref_fasta):
    ...
    for query_name, aln_list in high_identity_alignments.items():
        ...
        for i in range(len(sorted_alns) - 1):
            for j in range(i+1, len(sorted_alns)):
                ...
                if identity1 < 0.95 and identity2 < 0.95:
                    continue
                ...
                if ("head" in ref1 and strand1 == "+") or ("tail" in ref1 and strand1 == "-"):
                    # # then ref1 should be first
                    # if ("tail" in ref2 and strand2 == "+") or ("head" in ref2 and strand2 == "-"):
                    #     gap_size = -(40000 + (q_start2 - q_end1))
                    # else:
                    #     continue
                    continue
    ...
    for ref_pair, entries in ref_pair_connections.items():
        print(f"\n== Ref pair (grouped as): {ref_pair[0]} <--> {ref_pair[1]} ({len(entries)} supporting reads)")
...
def main():
    ...
    if not os.path.isfile(args.output_bam):
        run_minimap2_to_bam(flank_fasta, args.hifi_reads, args.output_bam)
    
    high_identity_alignments = filter_alignments_with_identity(args.output_bam, args.threshold)
    summarize_full_connected_sequences(high_identity_alignments, flank_fasta)
