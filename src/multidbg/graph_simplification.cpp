    ...
    for (auto&& node : this->graph) {
        for (auto&& node2 : node.second.outgoing_edges) {
            ...
            if (!incoming_tip.empty() && !outgoing_tip.empty() && outgoing_tip != node2.first && incoming_tip != node.first) {
                ...
                if (similarity < 0.9)
                    continue;
                std::cout << "[RepairTip] Broken bulge (and reverse complementary): " << node.first << "->" << node2.first << ", " << node.first << "->" << outgoing_tip << " " << incoming_tip << "->" << node2.first << ", sim " << similarity << std::endl;
...
void Graph::remove_whirl(Path& unambiguous_path, std::vector<std::string>& nodes_to_remove) {
    ...
    if (unambiguous_path.safe_to_extract) {
        ...
        if (multi_r != multi) {
            ...
            throw std::runtime_error("Error: inconsistent multiplicity in reverse complementary whirl removal");
        }
...
double Graph::matches_by_edlib(std::string sequence1, std::string sequence2, bool lcs) {
    ...
    if (result.status == EDLIB_STATUS_OK) {
        ...
        throw std::runtime_error("Error: edlib failed");
    }
}
...
bool Graph::get_reverse_path(Path& path, Path& path_reverse) {
    ...
    if (path.min_multi - path_reverse.min_multi > MIN_MULTI) {
        ...
        throw std::runtime_error("Error: inconsistent multiplicity in reverse complementary path");
    }
        ...
        if (p2_reverse.nodes == p1.nodes) {
            if (p1.nodes.size() > 2) {
                ...
                for (int i = 0; i + 1 < p1.nodes.size(); ++i) {
                    ...
                    bool flag_outgoing = false;
                    for (auto&& e : graph[node1].outgoing_edges) {
                        if (e.first != node2 && e.first != node1) {
                            flag_outgoing = true;
                            break;
                        }
                    }
                    bool flag_incoming = false;
                    for (auto&& e : graph[node2].incoming_edges) {
                        if (e.first != node1 && e.first != node2) {
                            flag_incoming = true;
                            break;
                        }
                    }
                ...
                if (index_to_remove == -1) {
                    ...
                    if (graph[p1.nodes[index_to_remove]].outgoing_edges[p1.nodes[index_to_remove + 1]].at(0).sequence.size() >= 10000000) // skip too long paths
                        continue;
                    std::cout << "[Decouple] Palindromic bulge: path1 (length " << p1.length << ", min multi " << p1.min_multi << "): " << p1.nodes[0];
                    for (int i = 1; i < p1.nodes.size();++i)
                        std::cout << "->" << p1.nodes[i];
                    std::cout << "; path2 (length " << p2.length << ", min multi " << p2.min_multi << "): " << p2.nodes[0];
                    for (int i = 1; i < p2.nodes.size();++i)
...
void Graph::resolve_edges_rc(std::string node1, std::string node2, int& resolved_edges, std::vector<std::string>& nodes_to_remove, bool strict) {
    ...
    if (node1 != node2)
        std::cout << "[Decouple] Decouple strands: " << "path1: " << incoming_nodes[0] << "->" << node1 << "->...->" << node2 << "->" << outgoing_nodes[1] << " multi " << path1.multiplicity << "; path2: " << incoming_nodes[1] << "->" << node1 << "->...->" << node2 << "->" << outgoing_nodes[0] << " multi " << path2.multiplicity << std::endl;
    else {
        std::cout << "[Decouple] Decouple strands: " << "path1: " << incoming_nodes[0] << "->" << node1 << "->" << outgoing_nodes[1] << " multi " << path1.multiplicity << "; path2: " << incoming_nodes[1] << "->" << node1 << "->" << outgoing_nodes[0] << " multi " << path2.multiplicity << std::endl;
    }
    ...
    if (path_1_r.nodes != path2.nodes) {
        ...
        if (node1 != node2)
            std::cout << "[Decouple] Decouple strands: " << "path1: " << in_rev[0] << "->" << reverse_complementary_node(node2) << "->...->" << reverse_complementary_node(node1) << "->" << out_rev[0] << " multi " << path_1_r.multiplicity << "; path2: " << in_rev[1] << "->" << reverse_complementary_node(node2) << "->...->" << reverse_complementary_node(node1) << "->" << out_rev[1] << " multi " << path_2_r.multiplicity << std::endl;
        else {
            std::cout << "[Decouple] Decouple strands: " << "path1: " << in_rev[0] << "->" << reverse_complementary_node(node2) << "->" << out_rev[0] << " multi " << path_1_r.multiplicity << "; path2: " << in_rev[1] << "->" << reverse_complementary_node(node2) << "->" << out_rev[1] << " multi " << path_2_r.multiplicity << std::endl;
        }
...
void Graph::remove_items_from_vector(std::vector<int>& vec, const std::vector<int>& discontinued_indices) {
...
void Graph::remove_low_coverage_edges(unsigned& removed_edges, double coverage, bool tips, bool force, bool avoid_chromosome) {
    ...
    if (avoid_chromosome) {
        std::cout << "[RemoveLow] " << "Avoid chromosome is triggered, will only remove edges of coverage <= " << coverage << " in components of mean coverage >= " << mean_cov * 10 << " and long edge length <= 1 Mbp" << std::endl;
    }
    // if (avoid_chromosome) {
    ...
    for (auto&& c : comp_to_cov) {
        ...
        if (avoid_chromosome)
            std::cout << "[RemoveLow] Comp " << c.first << " has average coverage " << comp_to_avgcov[c.first] << ", long edge length " << comp_to_len[c.first] << ", high-cov edges " << comp_to_high_cov_edges[c.first] << std::endl;
    }
    // }
...
void Graph::remove_chimeric_edge(std::string chimeric_path) {
    std::ifstream fin(chimeric_path);
    std::string line;
    std::unordered_set<std::string> chimeric_edges;
    while (getline(fin, line)) {
        std::cout << "[RemoveChimeric] Chimeric edge to remove: " << line << std::endl;
    ...
    for (auto&& node : graph) {
        ...
        for (auto&& n2 : node.second.outgoing_edges) {
            ...
            for (int i = 0; i < graph[n2.first].incoming_edges[node.first].size(); ++i) {
                if (chimeric_edges.find(graph[n2.first].incoming_edges[node.first].at(i).label) != chimeric_edges.end()) {
                    indices.push_back(i);
                }
            }
        ...
        for (auto&& n2 : node.second.incoming_edges) {
            ...
            for (int i = 0; i < graph[n2.first].outgoing_edges[node.first].size(); ++i) {
                if (chimeric_edges.find(graph[n2.first].outgoing_edges[node.first].at(i).label) != chimeric_edges.end()) {
                    indices.push_back(i);
                }
            }
...
void Graph::extract_unambiguous(unsigned& num_paths) {
    ...
    for (auto&& n : graph) {
        // incoming edge is not necessary a tip
        // if (!n.second.incoming_edges.empty())
        //     continue;
        // if (n.second.outgoing_edges.size() != 1) {
        //     if (n.second.outgoing_edges.size() != 2 || n.second.outgoing_edges.find(n.first) == n.second.outgoing_edges.end())
        //         continue;
        // }

        std::string out_node;
        for (auto&& n_o : n.second.outgoing_edges) {
            if (n_o.first != n.first) {
                out_node = n_o.first;
        ...
        if (graph.find(node1) != graph.end() && graph.find(node2) != graph.end() && graph.find(node3) != graph.end()) {
            if (graph[node2].outgoing_edges.find(node3) != graph[node2].outgoing_edges.end() && graph[node2].outgoing_edges[node3].at(0).sequence.size() >= 200000) {
                ...
                if (graph[node2].outgoing_edges[node3].at(0).sequence.size() >= 5000000) {
                    if (cnt_tip >= 2)
                        continue;
                    if (cnt_tip == 1 && !graph[node1].incoming_edges.empty())
                        continue;
                    if (cnt_tip == 0)
                        continue;
                }
