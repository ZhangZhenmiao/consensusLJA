...
void MDBGRunner::simplifyMDBG() {
    ...
    //     removed_paths = 1;
    //     while (removed_paths) {
    //         graph.resolving_bulge_with_two_multi_edge_paths(removed_paths, 5, 0.6, true, 2, true);
    //         if (removed_paths)
    //             flag = false;
    //         if (removed_paths > 0)
    ...
    // Only for debugging
    Graph graph;
    graph.restart_from_dot(output + "/graph.decoupling.dot", output + "/graph.decoupling.fasta");
    // Only for debugging

    std::cout << "----------Stage 7: decoupling strands further----------" << std::endl;
    // total_removed = 0;
    // while (true)
    // {
    //     bool flag = true;
    //     removed_paths = 1;
    //     while (removed_paths) {
    //         graph.resolving_bulge_with_two_multi_edge_paths(removed_paths, 6, 0.6, true, 2, true);
    //         if (removed_paths)
    //             flag = false;
    //         if (removed_paths > 0)
    //             std::cout << "Detoured " << removed_paths << " paths" << std::endl;
    ...
    std::cout << "Start removing complex palindromic bulges" << std::endl;
    ...
    std::cout << "Removed 2-in-2-out: " << total_removed << std::endl;
    graph.write_graph(output + "/graph.decoupling2");
    // graph.get_annotation(output + "/graph.decoupling");
    // graph.write_graph_contracted(output + "/graph.decoupling.contracted.20k", 20000);
