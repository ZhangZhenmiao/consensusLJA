...
void Graph::write_graph_gfa(const std::string& prefix) {
            ...
            for (auto&& e_o : n_o.second) {
                for (auto&& n_i : n.second.incoming_edges) {
                    for (auto&& e_i : n_i.second) {
                        ...
                        file_gfa << "L\t" << edgeid2gfa_node[e_i.label].first << "\t" << edgeid2gfa_node[e_i.label].second << "\t" << edgeid2gfa_node[e_o.label].first << "\t" << edgeid2gfa_node[e_o.label].second << "\t5001M\n";
                    }
                }
            }
        }
    ...
    for (auto&& r : reads) {
        file_aln << r << " ";
        if (read2aln[r].nodes_path.size() >= 2) {
            file_aln << read2aln[r].nodes_path[0] << " P:" << read2aln[r].start_base_path << " " << read2aln[r].prefix << " " << read2aln[r].suffix << '\n';
        }
        else {
            assert(read2aln[r].nodes_path.size() == 0);
            file_aln << "0 P: 0 0\n";
        }
    }

    file_aln << pseudo2aln.size() << '\n';

    for (auto&& r : pseudo2aln) {
        file_aln << r.first << " ";
        if (r.second.nodes_path.size() >= 2) {
            file_aln << r.second.nodes_path[0] << " P:" << r.second.start_base_path << " " << r.second.prefix << " " << r.second.suffix << '\n';
        }
        else {
            assert(r.second.nodes_path.size() == 0);
            file_aln << "0 P: 0 0\n";
        }
    }
